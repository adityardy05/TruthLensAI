...
from team_b.nodes.confidence_gate  import confidence_gate_node, route_after_gate
from team_b.nodes.round2_qa        import round2_qa_node
from team_b.nodes.final_judgment   import final_judgment_node

CHECKPOINT_DB = os.getenv("CHECKPOINT_DB", "checkpoints/truthlens_checkpoints.db")
...
def build_graph(use_checkpointing: bool = True):
    ...
    Args:
        use_checkpointing: Save state after each node (enables crash recovery)
    ...
    Graph structure:
        ...
        round1_qa
            ↓
        confidence_gate ──── conf >= 0.80 ──→ final_judgment
            │                                       ↑
            └─── conf < 0.80 ──→ round2_qa ─────────┘
    """
def build_graph(use_checkpointing: bool = True):
