...
class TestDecomposeClaimNode:
    ...
    def test_normal_claim(self):
        ...
        with patch("backend.agents.team_b.nodes.decompose_claim.call_deepseek_json",
...
class TestRound1QA:
    ...
    def test_runs_all_3_personas(self):
        ...
        with patch("backend.agents.llm_client.call_deepseek", return_value=FAKE_QUESTION):
...
class TestConfidenceGate:
    ...
    def test_high_confidence_skips_round2(self):
        ...
        from backend.agents.team_b.nodes.confidence_gate import confidence_gate_node

        state = _after_round1(_base_state(), certain_answers=True)
        state["avg_source_quality"] = 1.0
        state["high_quality_count"] = 3
    ...
    def test_route_function_returns_correct_node(self):
        ...
        from backend.agents.team_b.nodes.confidence_gate import route_after_gate
...
class TestFinalJudgment:
    ...
    def test_false_verdict_for_contradicting_evidence(self):
        ...
        with patch("backend.agents.team_b.nodes.final_judgment.call_deepseek_json",
...
class TestFullPipeline:
    ...
    def test_round1_only_path(self):
        ...
        with patch("backend.agents.team_b.nodes.decompose_claim.call_deepseek_json", return_value=FAKE_DECOMPOSE_RESPONSE), \
             patch("backend.agents.llm_client.call_deepseek", return_value=FAKE_ANSWER), \
             patch("backend.agents.team_b.nodes.final_judgment.call_deepseek_json", return_value=FAKE_JUDGMENT_RESPONSE):
