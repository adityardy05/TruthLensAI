...
function exportReport() {
    ...
    setTimeout(() => {
        ...
        const claim = (hasCurrentClaim() && getCurrentClaim().text)
            ...
            || getCurrentClaimText()
            || 'Claim not available';
        const verdict = readVerdictFromResults();
        ...
        doc.text(`Confidence:   ${REPORT_META.confidence}`, 20, y); y += 6;
        doc.text(`Language:     ${language}`, 20, y); y += 6;
        doc.text(`Sources:      ${REPORT_META.sources}`, 20, y); y += 6;
        doc.text(`Sub-claims:   ${REPORT_META.subclaims}`, 20, y); y += 8;
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        REPORT_SUBCLAIMS.forEach(([num, label, sub]) => {
            ensureSpace(5);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        REPORT_SOURCES.forEach(([domain, score, rating, rel]) => {
            ensureSpace(0);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        REPORT_AGENTS.forEach(([name, finding]) => {
