...
def test_rag_claim(claim_text: str, original_lang: str = "en"):
    ...
    raw_demo_evidence = [
        {
            ...
            "source_type": "web",
            "text_snippet": "Drinking alcohol does not protect against COVID-19 and can be dangerous. Consuming alcohol or whiskey does not cure virus infection.",
        ...
        {
            "source_id": "web_2",
            "source_url": "https://www.reuters.com/article/uk-factcheck-alcohol-covid19",
            "title": "Fact Check: Alcohol does not cure COVID-19",
            "source_type": "web",
            "text_snippet": "False claim: Drinking high-proof alcohol or whiskey kills coronavirus in the throat or body. Health officials confirm alcohol consumption offers no protection.",
        ...
        {
            "source_id": "web_3",
            "source_url": "http://viral-news24-7breaking.xyz/whiskey-cure",
            "title": "Shocking secret remedy: Whiskey cures COVID",
            "source_type": "web",
            "text_snippet": "Whiskey destroys all viruses instantly in your body when taken daily.",
    ...
    t0 = time.time()
    scored_evidence = pipeline.score_and_rank_evidence(claim_text, raw_demo_evidence, top_k=5)
    t1 = time.time()
    ...
    result = {
        ...
        "metadata": {
            ...
            "top_source_domain": scored_evidence[0]["source_domain"] if scored_evidence else "N/A"
        }
    }

    print(f"\n[RAG Engine] Retrieval & 4-Factor R(d) Scoring Completed in {(t1 - t0)*1000:.2f} ms")
    ...
    print(f"  Total Sources Found : {result['metadata']['num_sources_total']}")
    print(f"  Top Sources Kept   : {result['metadata']['num_sources_kept']}")
    print(f"  Top Source Domain  : {result['metadata']['top_source_domain']}")

    print("\n--- RETRIEVED EVIDENCE & 4-FACTOR R(d) SCORES ---")
    for item in result["evidence"]:
        print(f"\n[Rank #{item['retrieval_rank']}] Domain: {item['source_domain']} | Source Layer: {item['credibility_source']}")
        print(f"  - Combined Reliability R(d) : {item['combined_reliability']}")
        print(f"  - BM25 Score (s1)          : {item['bm25_score']}")
        print(f"  - Cosine Similarity (s2)   : {item['cosine_score']}")
        print(f"  - Domain Credibility (s3)  : {item['domain_credibility']}")
        print(f"  - Recency Score (s4)       : {item['recency_score']}")
...
if __name__ == "__main__":
    sample_claim = "Does drinking alcohol or whiskey cure COVID-19?"
